...
<template>
  <div
    class="p-4 rounded-lg border ml-16"
  ...
  >
    <div class="flex justify-between items-center mb-2">
      ...
      <button
        ...
        :class="theme === 'dark' ? 'bg-red-700 hover:bg-red-600' : 'bg-red-100 hover:bg-red-200'"
      >
        Обновить снизу
      </button>
    </div>

    <div class="p-3 rounded mt-2" :class="theme === 'dark' ? 'bg-gray-900' : 'bg-white'">
    ...
    <p class="text-xs opacity-80 mt-3">
      Этот компонент находится на 5-м уровне вложенности, но имеет прямой доступ к данным через
      inject без передачи через промежуточные компоненты.
    </p>
  </div>
</template>
