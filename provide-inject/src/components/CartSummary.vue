...
const mostExpensiveItem = computed(() => {
  ...
  return cart.items.reduce((max, item) => (item.price > max.price ? item : max), cart.items[0]);
});
</script>
