...
<template>
  <div>
    <h2 class="text-2xl font-bold text-gray-800 mb-6">
      Scoped слоты (данные из Дочернего компонента → рендер у родителя)
    </h2>

    <p class="text-gray-600 mb-8">
      Scoped slot — это slot, которому компонент передаёт параметры (например, <code>row</code>,
      <code>index</code>). Дочерний компонент управляет структурой, родитель — отображением.
    </p>

    <div class="space-y-8">
      <section class="bg-gray-50 p-6 rounded-xl">
        <h3 class="text-lg font-semibold text-gray-900 mb-3">
          1) Таблица: кастомизация отдельных ячеек
        </h3>
        ...
        <div class="mt-4 p-3 bg-blue-50 rounded-lg text-sm text-blue-800">
          DataTable не знает, как рисовать статус/кнопки — он только передаёт <code>row</code> в
          слот.
        </div>
      </section>
      ...
      <section class="bg-gray-50 p-6 rounded-xl">
        <h3 class="text-lg font-semibold text-gray-900 mb-3">
          2) Пагинированный список: item slot + controls slot
        </h3>
        ...
        <div class="mt-4 p-3 bg-green-50 rounded-lg text-sm text-green-800">
          Дочерний компонент делает пагинацию и передаёт родителю
          <code>items/page/totalPages</code>, а UI целиком у родителя.
        </div>
      </section>
    </div>
  </div>
