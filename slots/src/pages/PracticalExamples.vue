      ...
      <SmartListPanel :items="notifications">
        <template #header>
          <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
              <button class="px-3 py-1 border rounded hover:bg-white" @click="markAllRead">
                Прочитать все
              </button>
              <button class="px-3 py-1 border rounded hover:bg-white" @click="clearAll">
                Очистить
              </button>
            </div>
          </div>
        ...
        <template #footer>
          <div class="text-xs text-gray-500">
            Компонент управляет структурой (header/list/footer), родитель управляет рендером item и
            состояниями.
          </div>
        </template>
      </SmartListPanel>
