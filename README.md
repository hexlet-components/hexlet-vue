...
Примеры к урокам Vue, собранные монорепозиторием: на каждую тему свой маленький проект, который запускается отдельно.

Так сделано, чтобы пример не тонул в чужом коде: смотришь `pinia` — видишь только работу со стором, смотришь `virtual-dom` — только перерисовку. Список тем виден по каталогам: `async`, `optimization`, `pinia`, `provide-inject`, `router`, `slots`, `template-ref`, `virtual-dom`.
